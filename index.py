...
for option in (select.options):
    if not option.get_attribute("value"): continue
...
for venue_name, venue_no in venues:
    ...
    res = s.get(f'https://members.myactivesg.com/facilities/ajax/getTimeslots?activity_id=18&venue_id={venue_no}&date={date}', headers=headers)
    time.sleep(1)
    content = res.json()
